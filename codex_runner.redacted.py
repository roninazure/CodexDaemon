...
CodexDaemon Runner
------------------
Local + GitHub Actions compatible automation runner.

This script connects to OpenAI using your API key from .env or GitHub Secrets.
It performs code edits, generates new functions, and commits changes directly
to your CodexDaemon repository.

Usage Examples:
    python3 codex_runner.py "add a function to fetch weather data"
    python3 codex_runner.py "improve logging in codex_runner.py" --commit
...
# === Simple Greeting ===
def say_hello():
    console.print("[cyan]Hello, CodexDaemon![/cyan]")
...
def commit_push(repo, message, push=True):
    ...
    author_email = os.getenv("GIT_AUTHOR_EMAIL", "bot@codexdaemon.local")
...
def main():
    ...
    # Detect target file in instruction
